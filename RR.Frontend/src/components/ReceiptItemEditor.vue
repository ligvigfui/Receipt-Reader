<template>
    <div class="receipt-item-editor">
        ...
        <input type="number" v-model.number="localItem.quantity" min="0" step="0.01" style="width:5em" placeholder="Qty"
            @input="emitUpdate" />
        <select v-model="localItem.measurement" @change="emitUpdate">
            <option v-for="m in measurements" :key="m" :value="m">{{ m }}</option>
        </select>
        <input type="number" v-model.number="localItem.pricePerQuantity" min="0" step="0.01" style="width:6em"
            placeholder="Price/Qty" @input="emitUpdate" />
